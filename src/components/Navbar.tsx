      ...
      <div className="container flex h-16 items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          delay: i * 0.1
              ...
              {item.href.startsWith('/') && !item.href.includes('#') ? <Link to={item.href} className="text-sm font-medium transition-all hover:text-chittoor-green relative group">
                  {item.title}
                ...
                </Link> : <a href={item.href} className="text-sm font-medium transition-all hover:text-chittoor-green relative group">
                  {item.title}
                  <span className="absolute left-0 bottom-0 w-0 h-0.5 bg-gradient-to-r from-chittoor-green to-chittoor-blue group-hover:w-full transition-all duration-300"></span>
                </a>}
            </motion.div>)}
        ...
        <Sheet>
          ...
          <SheetContent>
            <div className="flex flex-col gap-4 mt-8">
              {navItems.map(item => <React.Fragment key={item.title}>
                  {item.href.startsWith('/') && !item.href.includes('#') ? <Link to={item.href} className="text-lg font-medium transition-colors hover:text-chittoor-green">
                      {item.title}
                    </Link> : <a href={item.href} className="text-lg font-medium transition-colors hover:text-chittoor-green">
                      {item.title}
                    </a>}
                </React.Fragment>)}
